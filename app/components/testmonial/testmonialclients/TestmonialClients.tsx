    ...
    return (
        <div className='flex flex-col items-center'>
            ...
            <div className='relative w-[100px] sm:w-[130px] h-[100px] sm:h-[130px] mb-10'>
                <Image src={data[index].src} alt='img' fill className='rounded-full border-4 border-testmonial-imageborder shadow-[0_0_20px] shadow-testmonial-imageshadow' />
            </div>
            <div className='flex gap-3 sm:gap-5 justify-center items-center flex-wrap mb-10 '>
                ...
                {data.map((img, i) => (
                    <div key={i} className='relative h-[40px] w-[40px] sm:h-[50px] sm:w-[50px]'>
                        <Image fill src={img.src} alt={img.name} className={`rounded-full shadow-[0_0_10px] shadow-testmonial-imageshadow transition-all duration-300 ${index === i ? '-translate-y-3 sm:-translate-y-4 border-2 border-testmonial-imageborder animate-pulse' : ''}`} />
                    </div>
                ))}
